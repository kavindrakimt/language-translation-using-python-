...
def parse_args(args=None):
    ...
    parser.add_argument('--finetune', action='store_true',
                        help='Load existing model during `train` mode from `save_dir` path')
    ...
    parser.add_argument('--charlm', action='store_true',
                        help="Turn on contextualized char embedding using pretrained character-level language model.")
    ...
    parser.add_argument('--char_lowercase', dest='char_lowercase', action='store_true',
                        help="Use lowercased characters in character model.")
    ...
    parser.add_argument('--no_input_transform', dest='input_transform', action='store_false',
                        help="Do not use input transformation layer before tagger lstm.")
...
def train(args):
    ...
    if args['finetune'] and args['finetune_load_name']:
        ...
        if pretrain is not None:
            ...
            if args['word_emb_dim'] and args['word_emb_dim'] != word_emb_dim:
                logger.warning(
                    "Embedding file has a dimension of {}.  Model will be built with that size instead of {}".format(
        ...
        if args['charlm']:
            ...
            if not args['charlm_forward_file']:
                args['charlm_forward_file'] = '{}/{}_forward_charlm.pt'.format(args['charlm_save_dir'],
                                                                               args['charlm_shorthand'])
            if not args['charlm_backward_file']:
                args['charlm_backward_file'] = '{}/{}_backward_charlm.pt'.format(args['charlm_save_dir'],
                                                                                 args['charlm_shorthand'])
    ...
    if trainer is None:  # init if model was not loaded previously from file
        # creates a new trainer which means it's setting up a new model iirc --> trainer.py call
        # trainer also is what creates the new model if there isn't one in trainer.model so this is where we are
        # setting up the LSTM --> consistent in all of the pipeline processes
    ...
    dev_score_history = []
    best_dev_preds = []  # what is best dev preds used for??
    current_lr = trainer.optimizer.param_groups[0]['lr']
    global_start_time = time.time()
    format_str = '{}: step {}/{}, loss = {:.6f} ({:.3f} sec/batch), lr: {:.6f}'

    # LR scheduling -> scheduled learning rate decay if given the decay arg, this is unique to NER
    if args['lr_decay'] > 0:
    ...
    # Pre-existing comment - start training
    train_loss = 0
    # basically just keep going with the batches until given reason to stop
    while True:
        should_stop = False
        for i, batch in enumerate(train_batch):
            start_time = time.time()
            global_step += 1
            # update step on the model --> back to trainer.py
            ...
            if global_step % args['eval_interval'] == 0:
                ...
                logger.info(
                    "step {}: train_loss = {:.6f}, dev_score = {:.4f}".format(global_step, train_loss, dev_score))
                train_loss = 0
...
def load_model(args, model_file):
    ...
    trainer = Trainer(args=charlm_args, model_file=model_file, use_cuda=use_cuda,
                      train_classifier_only=args['train_classifier_only'])
