...
class PyDataset(Dataset):
    """The same logic as DataLoader but as a torch dataset"""
    def __init__(self, args, input_files={'json': None, 'txt': None, 'label': None}, input_text=None, input_data=None,
                 vocab=None, evaluation=False, sentences=None):
        self.args = args
        self.eval = evaluation

        """"We need to add those attributes that were passed to next() as class attributes to be able to change them
        from batch to batch"""
        self.eval_offsets = None
        self.unit_dropout = 0.0
        self.old_batch = None
    ...
    def __len__(self):
        return int(len(self.sentence_ids)/32)
    ...
    def __getitem__(self, item):
        ...
        if self.old_batch is not None:
            ...
            # and just (essentially) advance the indices/offsets from where we read converted data in this old batch.
            # In this case, eval_offsets index within the old_batch to advance the strings to process.
            ounits, olabels, ofeatures, oraw = self.old_batch
            lens = (ounits != padid).sum(1).tolist()
            pad_len = max(l-i for i, l in zip(self.eval_offsets, lens))
            ...
            for i in range(len(ounits)):
                self.eval_offsets[i] = min(self.eval_offsets[i], lens[i])
                units[i, :(lens[i] - self.eval_offsets[i])] = ounits[i, self.eval_offsets[i]:lens[i]]
                labels[i, :(lens[i] - self.eval_offsets[i])] = olabels[i, self.eval_offsets[i]:lens[i]]
                features[i, :(lens[i] - self.eval_offsets[i])] = ofeatures[i, self.eval_offsets[i]:lens[i]]
                raw_units.append(oraw[i][self.eval_offsets[i]:lens[i]] + ['<PAD>'] * (pad_len - lens[i] + self.eval_offsets[i]))
        ...
        if self.eval_offsets is not None:
            # find max padding length
            pad_len = 0
            for eval_offset in self.eval_offsets:
                if eval_offset < self.cumlen[-1]:
                    pair_id = bisect_right(self.cumlen, eval_offset) - 1
                    pair = self.sentence_ids[pair_id]
                    pad_len = max(pad_len, len(strings_starting(pair, offset=eval_offset-self.cumlen[pair_id])[0]))
            ...
            pad_len += 1
            id_pairs = [bisect_right(self.cumlen, eval_offset) - 1 for eval_offset in self.eval_offsets]
            pairs = [self.sentence_ids[pair_id] for pair_id in id_pairs]
            offsets = [eval_offset - self.cumlen[pair_id] for eval_offset, pair_id in zip(self.eval_offsets, id_pairs)]
        ...
        if self.unit_dropout > 0 and not self.eval:
            # dropout characters/units at training time and replace them with UNKs
            mask = np.random.random_sample(units.shape) < self.unit_dropout
            mask[units == padid] = 0
            units[mask] = unkid
            for i in range(len(raw_units)):
                for j in range(len(raw_units[i])):
